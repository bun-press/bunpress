...
describe('Theme Registry Plugin', () => {
  ...
  const TEST_DIR = path.join(process.cwd(), '__test-themes__');
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(VALID_THEME_DIR, 'index.tsx'), 'export default function Layout() { return null; }');
    ...
    fs.writeFileSync(path.join(INVALID_THEME_DIR, 'index.tsx'), 'export default function Layout() { return null; }');
  ...
  it('should register valid themes correctly', () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Theme Registry: Registering themes'));
  ...
  it('should handle custom themes directory', () => {
    ...
    fs.writeFileSync(path.join(CUSTOM_THEME_DIR, 'index.tsx'), 'export default function Layout() { return null; }');
