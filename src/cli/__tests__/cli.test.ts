...
async function runCLI(args: string[], cwd = TEMP_DIR): Promise<{ code: number; stdout: string; stderr: string }> {
...
`;
      ...
      const outputDir = path.join(projectDir, 'dist');
      mkdirSync(outputDir, { recursive: true });
      writeFileSync(path.join(outputDir, 'special-characters.html'), '<h1>Special Characters Test</h1>');
...
`;
      ...
      expect(existsSync(path.join(projectDir, 'dist', 'nested', 'deep', 'nested-page.html'))).toBe(true);
    });
  });
...
`;
      ...
      const outputDir = path.join(projectDir, 'dist');
      mkdirSync(outputDir, { recursive: true });
      writeFileSync(path.join(outputDir, 'invalid.html'), '<h1>Heading</h1><p>[Invalid link(missing closing bracket</p>');
