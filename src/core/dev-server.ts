...
import { fileExists, readFileAsString } from '../lib/fs-utils';

import { joinPaths } from '../lib/path-utils';

import { ServerConfig, openBrowser, handleStaticFileRequest } from '../lib/server-utils';

import { createFileWatcher, FileWatcher } from '../lib/watch-utils';

import { ErrorCode, tryCatch, BunPressError } from '../lib/error-utils';
  ...
  return await tryCatch(
    async () => {
      ...
      const serverConfig: DevServerConfig = {
        ...
        hmrPort:
          devConfig.hmrPort ||
          (process.env.NODE_ENV === 'test' ? Math.floor(Math.random() * 10000) + 50000 : 3030),
      ...
      logger.info(
        chalk.cyan(
          `Starting development server at http://${serverConfig.hostname}:${serverConfig.port}`
        )
      );
      ...
      const server = Bun.serve({
        ...
        websocket: {
          ...
          message:
            hmrContext.websocket.message ||
            ((_ws: ServerWebSocket<unknown>, message: string | Uint8Array) => {
              try {
